...
if nodo_a is None:
    print("❌ No se encontró un nodo con DEF 'bloqueA'")
else:
    posicion_a = nodo_a.getField("translation")
    rotacion_a = nodo_a.getField("rotation")

if nodo_b is None:
    print("❌ No se encontró un nodo con DEF 'bloqueB'")
else:
...
while control.step(paso_tiempo) != -1:
    ...
    distancia_base = math.sqrt(
        posicion_actual_a[0]**2 +
        posicion_actual_a[1]**2 +
        posicion_actual_a[2]**2
    )
    ...
    if nodo_b is not None:
        distancia_entre_bloques = math.sqrt(
        ...
        if distancia_entre_bloques < (radio_a + radio_b + distancia_minima):
            print("⚠️ Colisión detectada entre bloqueA y bloqueB")
